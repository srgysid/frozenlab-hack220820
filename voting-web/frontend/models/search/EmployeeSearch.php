...
class EmployeeSearch extends Model
{
    ...
    {
        return [
            [['id', 'fio', 'phone', 'user_profile_id', 'username', 'email', 'roles'], 'safe'],
        ];
    }
    ...
    public function search($params)
    {
        ...
        $query->addSelect([
            'user.id',
            new Expression("CONCAT(user_profile.second_name, ' ', user_profile.first_name, ' ', user_profile.third_name) as fio"),
...
//        $query->andFilterWhere(['IN', 'user_staff_pos.staff_pos_id', $this->staff_pos_ids]);
        ...
        $query->andFilterWhere(['ilike', 'user.email', $this->email]);
